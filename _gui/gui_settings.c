...
#ifdef _M_X64
#  define TFTPD_SETTINGS_TITLE   "Tftpd64: ÉèÖÃ  ºº»¯£ºbaimo"
#else
#  define TFTPD_SETTINGS_TITLE   "Tftpd32: ÉèÖÃ  ºº»¯£ºbaimo"
#endif


#define  RESET_DEFAULT_TEXT  "ÖØÖÃµ±Ç°ÅäÖÃ\n²¢Ïú»Ù×¢²á±íÏî ?"
...
{
	ONGLET_SETTINGS_GLOBAL,  "È«¾Ö",   "default",
...
// for (Ark=0 ; Ark<SizeOfTab(tResize) ; Ark++)
  //      ShowWindow (GetDlgItem (hWnd, tResize[Ark].idCtrl), SW_SHOW );
     ...
     sNewSettings = sGuiSettings;
	  ...
	  {
            MY_WARNING ("IPv6Î´ÕýÈ·Ö§³Ö\nWindows XP ºÍ Windows Server 2003");
  ...
  //      Settings_ChangeTabControl (hWnd);
        ...
        {
            MY_WARNING ("TFTP: ³¬Ê±ºÍ¶Ë¿ÚÓ¦ÎªÊý×Ö£¬²»ÄÜÎª0");
            return FALSE;
        }
        ...
        else
        {
            ...
            else   MY_WARNING ("TFTP: ÎÞ·¨¸ü¸ÄÄ¿Â¼");
        }
     ...
     // uncheck other choice
       ...
       case IDC_CHECK_SAVE_SYSLOG :
        ...
        case IDC_CHECK_WINSIZE :
           if ( ISDLG_CHECKED (hWnd, wParam) )
            {
               MY_WARNING ("Ê¹ÓÃÔ¤ÆÚ´°¿Ú¼ÓËÙ´«Êä\nÁíÒ»·½Ãæ£¬Ëü¿ÉÄÜ²»ÊÊÓÃÓÚÄúµÄTFTP¿Í»§¶Ë¡£");
        ...
        case IDC_BUTTON_DEFAULT :
             if (CMsgBox (hWnd, RESET_DEFAULT_TEXT, "Tftpd32", MB_YESNOCANCEL | MB_ICONQUESTION) == IDYES)
             {
                 ...
                 MY_WARNING ("ÉèÖÃÌõÄ¿ÒÑ±»É¾³ý¡£\nÄú±ØÐëÖØÐÂÆô¶¯Tftpd32²ÅÄÜÊ¹ÓÃÐÂµÄÉèÖÃ");
        ...
        case IDC_TFTPD_HELP:
            ...
            ShellExecute (hWnd, "´ò¿ª", szTftpd32Help,  NULL, NULL, SW_NORMAL);
            break;
